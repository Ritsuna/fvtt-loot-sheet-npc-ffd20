...
Hooks.on("ready", async () => {
  LootSheetConstants.LootSheetFFD20NPC = (await import("./modules/lootsheet-npc.js")).LootSheetFFD20NPC;
  
  //Register the loot sheet
  Actors.registerSheet("FFD20", LootSheetConstants.LootSheetFFD20NPC, {
    types: ["npc"],
    makeDefault: false
  });
});
...
Hooks.on('preCreateOwnedItem', (actor, item, data) => {
  ...
  if (actor.sheet instanceof LootSheetConstants.LootSheetFFD20NPC) {
...
Hooks.on("getActorDirectoryEntryContext", (html, options) => {
  options.push({
    ...
    condition: li => {
      const actor = game.actors.get(li.data("entityId"))
      return game.user.isGM && actor && actor.data.type === "npc" && !(actor.sheet instanceof LootSheetConstants.LootSheetFFD20NPC) && actor.data.token.actorLink;
    },
  });
